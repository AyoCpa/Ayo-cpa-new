...
import { v4 } from "uuid";
import { BlogType, BlogTypeEntry, NuggetType } from "@/types";
...
const EditBlog = () => {
  ...
  const [blogEntry, setBlogEntry] = useState<BlogTypeEntry>();
  ...
  useEffect(() => {
    const blogId = new URLSearchParams(window.location.search);
    const id = blogId.get("blogId");

    apiClient("get", `blog/getBySlug/${encodeURIComponent(id ? id : "")}`)
      .then((res) => {
        setModel(res?.data.content);
        setBlog(res?.data);
        setBlogEntry({
          ...res?.data,
          tags: res?.data.tags.map((item: NuggetType) => item._id),
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const imageUrl_ = imageChosen ? await uploadToFirebase() : blogEntry.image;
              ...
              >
                <form onSubmit={handleSubmit}>
                  ...
                  <div className="my-4">
                    <div>
                      <input
                        // value={blogEntry.isFeatured ? true : false}
                        type="checkbox"
                        checked={blogEntry?.isFeatured ? true : false}
                        name="isFeatured"
                        onChange={(e) => {
                          if (blogEntry) {
                            setBlogEntry({
                  ...
                  <div className="my-4">
                    ...
                    <div>
                      <Select
                        ...
                        className="py-2"
                        value={blogEntry?.tags}
                        onChange={(e) => {
                          if (blogEntry) {
                            setBlogEntry({ ...blogEntry, tags: e });
                          }
                        }}
